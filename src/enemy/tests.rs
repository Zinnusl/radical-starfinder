...
fn radical_action_radical_roundtrips_all_57_variants() {
    ...
    for action in &all_actions {
        let r = action.radical();
        assert!(!r.is_empty(), "radical() empty for {:?}", action);
        let recovered = RadicalAction::from_radical(r);
        assert_eq!(
            recovered,
            Some(*action),
            "from_radical(radical()) roundtrip failed for {:?} (radical={})",
            action,
            r
        );
    }
...
#[test]
fn radical_action_description_non_empty_for_all_variants() {
    let all_actions = [
        RadicalAction::SpreadingWildfire,
        RadicalAction::ErosiveFlow,
        RadicalAction::OverwhelmingForce,
...
#[test]
fn radical_action_damage_info_non_empty_for_all_variants() {
    let all_actions = [
        RadicalAction::SpreadingWildfire,
        RadicalAction::ErosiveFlow,
        RadicalAction::OverwhelmingForce,
    ...
    for action in &all_actions {
        assert!(!action.damage_info().is_empty(), "damage_info() empty for {:?}", action);
    }
}

// ── PlayerRadicalAbility: comprehensive coverage for all match arms ───────────

#[test]
fn player_radical_ability_radical_roundtrips_all_variants() {
    let all_abilities = [
        PlayerRadicalAbility::FireStrike,
        PlayerRadicalAbility::TidalSurge,
        PlayerRadicalAbility::PowerStrike,
    ...
    for ability in &all_abilities {
        let r = ability.radical();
        assert!(!r.is_empty(), "radical() empty for {:?}", ability);
        let recovered = PlayerRadicalAbility::from_radical(r);
        assert_eq!(
            recovered,
            Some(*ability),
            "from_radical(radical()) roundtrip failed for {:?} (radical={})",
...
#[test]
fn player_radical_ability_description_non_empty_for_all_variants() {
    let all_abilities = [
        PlayerRadicalAbility::FireStrike,
        PlayerRadicalAbility::TidalSurge,
        PlayerRadicalAbility::PowerStrike,
